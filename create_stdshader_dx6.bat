...
ECHO "Generating stdshader_dx6 solution for x86"
...
devtools\bin\vpc.exe /2022 /define:WORKSHOP_IMPORT_DISABLE /define:SIXENSE_DISABLE /define:NO_X360_XDK /define:RAD_TELEMETRY_DISABLED /define:DISABLE_ETW /define:NO_STEAM /define:NO_ATI_COMPRESS /define:NO_NVTC /define:LTCG /no_ceg /nofpo +tier0 +tier1 +vstdlib +mathlib +shaderlib +stdshader_dx6 /mksln stdshader_dx6.sln
if ERRORLEVEL 1 (
  ECHO VPC for stdshader_dx6.sln failed.
  EXIT /B 1
)

ECHO.
ECHO Successfully generated stdshader_dx6.sln
ECHO Open stdshader_dx6.sln in Visual Studio to build.
